...
class ToolsHandler:
    ...
    def handler_get_all_tools():
        try:
            ...
            return Response(CustomError.get_error_by_code('TO-0'), status=status.HTTP_400_BAD_REQUEST)
    ...
    def handler_get_all_tools_models():
        try:
            ...
            return Response(CustomError.get_error_by_code("TM-3"), status=status.HTTP_400_BAD_REQUEST)
    ...
    def handler_get_tool(tool_id):
        ...
        except ToolsModel.DoesNotExist as e:
            return Response(CustomError.get_error_by_code("TO-1", e), status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
            return Response(CustomError.get_error_by_code("GE-0", e), status=status.HTTP_400_BAD_REQUEST)
    ...
    def handler_get_tools_history_by_user(user):
        try:
            ...
            if history_objects:
                ...
                return Response(CustomError.get_error_by_code("TH-1"),
    ...
    def handler_add_tool(request):
        try:
            ...
            if not tool_model:  # tool model doesn't exist, create one
                ...
                if new_tool_model.is_valid():
                    ...
                    return Response(CustomError.get_error_by_code("TM-4", new_tool_model.errors),
                                    status=status.HTTP_400_BAD_REQUEST)
            else:
            ...
            if new_tool.is_valid():
                ...
                return Response(CustomError.get_error_by_code("TO-2", new_tool.errors),
                                status=status.HTTP_400_BAD_REQUEST)
    ...
    def handler_update_tool(tool_id, request):
        try:
            tool = ToolsHelper.get_tool_by_id(tool_id)
            if not tool:
                return Response(CustomError.get_error_by_code("TO-1"), status=status.HTTP_400_BAD_REQUEST)
                ...
                if tool_model.exists() is False:  # tool model doesn't exist, create one
                    ...
                    if tool.model == tool_model:  # Update the tools model values
                        ...
                        if tool_model_serializer.is_valid():
                            ...
                            return Response(CustomError.get_error_by_code("TM-1", tool_model_serializer.errors),
            ...
            if tool_serializer.is_valid():
                ...
                return Response(CustomError.get_error_by_code("TO-2", tool_serializer.errors),
                                status=status.HTTP_400_BAD_REQUEST)
